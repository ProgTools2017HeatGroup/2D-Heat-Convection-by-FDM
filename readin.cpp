 ...
 *
 *
 * This function will read in the parameters from input file and write
 * these values to a log file, and print them out to the screen in
 * scientific form at the meantime. The usage is: ./readin parameters.in
 */
...
int print_matrix (string str[100], string nam, int i, float(&matrix)[3][5], ofstream &file)
{
    ...
    for (j = 0; j < 3; j = j + 1)
    {
        ...
        {
            c = const_cast<char*>(str[i + 2].c_str());
            sscanf(c, "%e", &matrix[j][k]);
            file << scientific << matrix[j][k] << "\t";
            cout << scientific << matrix[j][k] << "\t";
            i++;
        }
        file << "\n";
        cout << "\n";
...
//  open the log file, and wtite the input values in float format
    ...
    while (str[i] != "")
    {
        if (str[i] == "density")
        {
            print_value(str, nam, i, rho, logfile);
        }
        if (str[i] == "viscosity")
        {
            print_value(str, nam, i, v, logfile);
        }
        if (str[i] == "diffusivity")
        {
            print_value(str, nam, i, k, logfile);
        }
        if (str[i] == "expansion_coefficient")
        {
            print_value(str, nam, i, a, logfile);
        }
        if (str[i] == "x_extend")
        {
            print_value(str, nam, i, xe, logfile);
        }
        if (str[i] == "y_extend")
        {
            print_value(str, nam, i, ye, logfile);
        }

        if (str[i] == "Pressure")
        {
            print_matrix(str, nam, i, P, logfile);
        }
        if (str[i] == "Temperature")
        {
            print_matrix(str, nam, i, T, logfile);
        }
        if (str[i] == "Velocity_X")
        {
            print_matrix(str, nam, i, Vx, logfile);
        }
        if (str[i] == "Velocity_Y")
        {
            print_matrix(str, nam, i, Vy, logfile);
        }
